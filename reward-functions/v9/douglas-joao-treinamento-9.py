...
def reward_function(params):
    ...
    # 3) Velocidade — sempre premiar ir rápido, escala com curvatura
    ...
    if is_straight:
        ...
        steering_penalty = (abs(steering_angle) / 21.0) ** 2 * 0.4
    else:
        # Curva: premiar velocidade ideal mas ainda valorizar velocidade
        ideal = max(1.33, 3.0 - curve_diff * 0.06)
        speed_reward = max(1.0 - abs(speed - ideal) / 3.0, 0.0)
        # Bônus por ir acima do mínimo mesmo em curva
        speed_reward += (speed / 4.0) * 0.3
        steering_penalty = 0.0

    # 4) Penalidade gradual por velocidade baixa (em qualquer trecho)
    slow_penalty = max(0.4 - speed / 4.0, 0.0)
    ...
    # Reward ponderado — velocidade é prioridade
    reward = (
        0.40 * speed_reward
        + 0.30 * heading_reward
        + 0.10 * center_reward
    ...
    # Bônus de volta completa — cúbico
    if progress >= 99.0:
